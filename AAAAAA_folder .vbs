...
If Not FSO.FolderExists(NewFold) Then
    ...
    LaunchHTA "Папка создана: " & NewFold, "Успех"
Else
    ' Запускаем HTA-окно о том, что папка уже есть
    LaunchHTA "Папка " & NewFold & " уже существует!", "Внимание"
End If
...
Function GetHTAContent(msg, ttl)
    ...
    "    .message { font-size: 20px; color: #333; margin: 0 0 20px 0; }" & vbCrLf & _
    ...
    "  </style>" & vbCrLf & _
    "  <script>" & vbCrLf & _
    "    window.resizeTo(600, 400);" & vbCrLf & _
    "    window.moveTo((screen.width-600)/2, (screen.height-400)/2);" & vbCrLf & _
    "    setTimeout(function() { window.close(); }, 5000);" & vbCrLf & _
    "  </script>" & vbCrLf & _
    "</head>" & vbCrLf & _
    "<body>" & vbCrLf & _
    "  <div class='container'>" & vbCrLf & _
    "    <div class='message'>" & msg & "</div>" & vbCrLf & _
    "    <button onclick='window.close()'>Закрыть</button>" & vbCrLf & _
    "  </div>" & vbCrLf & _
    "</body>" & vbCrLf & _
    "</html>"
End Function
